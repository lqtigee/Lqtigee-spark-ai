...
- Public inline SSE follow-up risk: `MOBILE-PUBLIC-M003` observed a stopped-run persistence race where PostgreSQL later showed `EXITED` and `ProcessOutputPump` logged `RUN_ALREADY_FINISHED`; do not use PostgreSQL run status as sole stopped-run truth until a dedicated bug ticket fixes this race.
